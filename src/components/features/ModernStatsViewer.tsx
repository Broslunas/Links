...
interface ModernStatsViewerProps {
  linkId: string;
  linkSlug?: string;
  className?: string;
}
...
export function ModernStatsViewer({
  linkId,
  linkSlug,
  className = '',
}: ModernStatsViewerProps) {
  ...
  const handleExport = async (exportFormat: 'csv' | 'json') => {
    try {
      ...
      let filename = `analytics_${linkSlug || linkId}_${format(new Date(), 'yyyy-MM-dd')}.${exportFormat}`;
