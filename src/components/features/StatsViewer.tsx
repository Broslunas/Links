...
export function StatsViewer({ linkId, linkSlug, className = '' }: StatsViewerProps) {
    ...
    const handleDetailedExport = async (format: 'csv' | 'json') => {
        try {
            ...
            const response = await fetch(`/api/analytics/export/detailed?${params.toString()}`);
            ...
            // You could add a toast notification here
            alert('Error al exportar los datos detallados. Por favor, inténtalo de nuevo.');
        }
    };
        ...
        <div className={`space-y-6 ${className}`}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Total de clicks: <span className="font-semibold text-card-foreground">{stats.totalClicks}</span>
                        </p>
                    </div>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                ...
                <div className="bg-card rounded-lg border border-border p-6">
                    ...
                    <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={stats.clicksByCountry.slice(0, 10)} layout="horizontal">
                                <CartesianGrid strokeDasharray="3 3" className="opacity-30" />
            ...
            <div className="bg-card rounded-lg border border-border p-6">
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <div className="text-center p-4 bg-muted/50 rounded-lg">
                        <p className="text-2xl font-bold text-card-foreground">{stats.totalClicks}</p>
                        <p className="text-sm text-muted-foreground">Total Clicks</p>
                    </div>
                    <div className="text-center p-4 bg-muted/50 rounded-lg">
                        <p className="text-2xl font-bold text-card-foreground">{stats.clicksByCountry.length}</p>
                        <p className="text-sm text-muted-foreground">Países</p>
                    </div>
                    <div className="text-center p-4 bg-muted/50 rounded-lg">
                        <p className="text-2xl font-bold text-card-foreground">{stats.clicksByDevice.length}</p>
                        <p className="text-sm text-muted-foreground">Tipos de Dispositivo</p>
                    </div>
                    <div className="text-center p-4 bg-muted/50 rounded-lg">
                        <p className="text-2xl font-bold text-card-foreground">{stats.clicksByBrowser.length}</p>
