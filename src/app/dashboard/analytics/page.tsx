    ...
    const fetchAnalytics = async () => {
        ...
        try {
            ...
            const analyticsPromises = linksData.data.map(async (link) => {
                try {
                    const url = `/api/analytics/${link.id}?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`;
            ...
            let totalClicks = 0;
            const topLinks: Array<{ linkId: string; slug: string; clicks: number; title?: string }> = [];
            ...
            validResults.forEach((result) => {
                ...
                stats.clicksByCountry.forEach((countryData: any) => {
                    ...
                    clicksByCountryMap.set(countryData.country, existing + countryData.clicks);
                });

                stats.clicksByDevice.forEach((deviceData: any) => {
                    const existing = clicksByDeviceMap.get(deviceData.device) || 0;
                    clicksByDeviceMap.set(deviceData.device, existing + deviceData.clicks);
                });
            });

            const sortedTopLinks = topLinks.sort((a, b) => b.clicks - a.clicks).slice(0, 10);
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mt-4">Cargando analíticas...</p>
                </div>
            </div>
        );
    }
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="bg-card rounded-lg border border-border p-6">
                        <div className="text-center">
                            ...
                            <Button onClick={fetchAnalytics} variant="outline">Reintentar</Button>
                        </div>
                    </div>
                </div>
            </DashboardLayout>
            ...
            <div className="space-y-6">
                    ...
                    <div className="flex gap-2">
                        {DATE_RANGES.map((range) => (
                            <button
                                ...
                                className={`px-3 py-1 text-sm rounded-md transition-colors ${selectedRange.label === range.label
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                    <div className="bg-card rounded-lg border border-border p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <p className="text-sm font-medium text-muted-foreground">Enlaces Totales</p>
                                <p className="text-2xl font-bold text-card-foreground">{analyticsData?.totalLinks || 0}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-card rounded-lg border border-border p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <p className="text-sm font-medium text-muted-foreground">Clicks Totales</p>
                                <p className="text-2xl font-bold text-card-foreground">{analyticsData?.totalClicks || 0}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-card rounded-lg border border-border p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <p className="text-sm font-medium text-muted-foreground">Promedio por Enlace</p>
                                <p className="text-2xl font-bold text-card-foreground">
                                    {analyticsData?.totalLinks ? Math.round((analyticsData.totalClicks || 0) / analyticsData.totalLinks) : 0}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-card rounded-lg border border-border p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                <p className="text-2xl font-bold text-card-foreground">{links.filter(link => link.isActive).length}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {analyticsData && (
                    <div className="grid gap-6 lg:grid-cols-2">
                        <div className="bg-card rounded-lg border border-border p-6">
                            <h3 className="text-lg font-semibold text-card-foreground mb-4">Clicks por Día</h3>
                            <div className="h-64">
                        ...
                        <div className="bg-card rounded-lg border border-border p-6">
                            <h3 className="text-lg font-semibold text-card-foreground mb-4">Distribución por Dispositivo</h3>
                            <div className="h-64">
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie
                ...
                {analyticsData && (
                    <div className="grid gap-6 lg:grid-cols-2">
                        <div className="bg-card rounded-lg border border-border p-6">
                            <h3 className="text-lg font-semibold text-card-foreground mb-4">Enlaces Más Populares</h3>
                            <div className="space-y-3">
                                {analyticsData.topLinks.length > 0 ? (
                                    analyticsData.topLinks.map((link, index) => (
                                        <div key={link.linkId} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-medium text-card-foreground">{link.title || `/${link.slug}`}</p>
                                                    <p className="text-sm text-muted-foreground">/{link.slug}</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="font-bold text-card-foreground">{link.clicks}</p>
                                                <p className="text-xs text-muted-foreground">clicks</p>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    <p className="text-muted-foreground text-center py-8">No hay datos de clicks disponibles</p>
                                )}
                            </div>
                        </div>

                        <div className="bg-card rounded-lg border border-border p-6">
                            <h3 className="text-lg font-semibold text-card-foreground mb-4">Países con Más Clicks</h3>
                            <div className="h-64">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={analyticsData.clicksByCountry} layout="horizontal">
                                        <CartesianGrid strokeDasharray="3 3" className="opacity-30" />
                                        <XAxis type="number" className="text-xs" />
                                        <YAxis dataKey="country" type="category" width={60} className="text-xs" />
                                        <Tooltip
                                            formatter={(value: number) => [value, 'Clicks']}
                                            contentStyle={{
                                                backgroundColor: 'hsl(var(--card))',
