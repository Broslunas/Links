...
async function verifyDNSRecord(domain: string, expectedValue: string): Promise<boolean> {
  try {
    console.log(`Verificando DNS para dominio: ${domain}, valor esperado: ${expectedValue}`);
    ...
    if (!response.ok) {
      console.log(`Error en respuesta DNS: ${response.status} ${response.statusText}`);
      return false;
    }
    ...
    if (data.Answer) {
      const found = data.Answer.some((answer: any) => {
        ...
        return answerData && answerData.toLowerCase().includes(expectedValue.toLowerCase());
      });
    ...
    if (data.Status === 0 && !data.Answer) {
      console.log('No se encontraron registros CNAME, verificando registros A...');
...
async function verifyVercelDomain(domain: string): Promise<{ success: boolean; verified: boolean; error?: string }> {
  try {
    ...
    if (!verifyDomainResponse.ok) {
      ...
      return {
        success: false,
        verified: false,
        error: errorData.error?.message || 'Error verificando dominio en Vercel',
      };
    }
  ...
  try {
    ...
    if (domain.verificationAttempts >= domain.maxVerificationAttempts) {
      return NextResponse.json(
        { 
          error: { 
            message: 'Se han agotado los intentos de verificación. Contacta con soporte.' 
          } 
        },
        { status: 400 }
      );
    ...
    if (!vercelVerification.verified) {
      ...
      const expectedValue = domain.dnsRecords[0]?.value || 'cname.vercel-dns.com';
      ...
      return NextResponse.json(
        { 
          error: { 
            message: 'El dominio aún no está verificado en Vercel. Asegúrate de que los registros DNS estén configurados correctamente.',
            details: {
              domain: domain.fullDomain,
    ...
    return NextResponse.json({
      success: true,
      data: domainResponse,
      message: 'Dominio verificado exitosamente. El SSL se configurará automáticamente en unos minutos.',
    });
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (domainInfo.verified && !domainInfo.verification?.some((v: any) => v.reason)) {
          domain.sslStatus = 'active';
          domain.sslError = undefined;
        } else if (domainInfo.verification?.some((v: any) => v.reason)) {
          domain.sslStatus = 'error';
          domain.sslError = domainInfo.verification.find((v: any) => v.reason)?.reason;
        }
