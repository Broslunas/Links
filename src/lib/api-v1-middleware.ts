...
export async function withApiV1Middleware(
  ...
  try {
    ...
    if (enableRateLimit) {
      const rateLimitResult = await applyApiV1RateLimitByPath(
        request,
        pathname
      );
    ...
    if (enableRateLimit && context.rateLimitResult) {
      response.headers.set(
        'X-RateLimit-Remaining',
        context.rateLimitResult.remaining?.toString() || '0'
      );
      response.headers.set(
        'X-RateLimit-Reset',
        context.rateLimitResult.resetTime?.toString() || ''
      );
    }
...
function logApiError(
  ...
  const method = request.method;
  console.error(
    `[API-V1] ${requestId} ${method} ${pathname} - ERROR - ${duration}ms:`,
    error.message
  );
...
function logRateLimitViolation(
  ...
  const authHeader = request.headers.get('authorization');
  const identifier =
    authHeader && authHeader.startsWith('Bearer uls_') ? 'token' : ip;

  console.warn(
    `[API-V1] ${requestId} RATE_LIMIT_EXCEEDED ${method} ${pathname} - Identifier: ${identifier} - Remaining: ${rateLimitResult.remaining} - Reset: ${rateLimitResult.resetTime}`
  );
}
