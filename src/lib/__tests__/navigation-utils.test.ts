...
describe('Navigation Utils', () => {
  describe('isDashboardRoute', () => {
    ...
    it('should return false for non-dashboard routes', () => {
      ...
      expect(isDashboardRoute('/docs')).toBe(false);
    });
  });

  describe('isActiveNavItem', () => {
    const homeItem: NavigationItem = { label: 'Home', href: '/' };
    const featuresItem: NavigationItem = {
      label: 'Features',
      href: '/features',
    };
  ...
  describe('shouldUseGlobalLayout', () => {
    it('should return true for non-dashboard routes', () => {
      ...
      expect(shouldUseGlobalLayout('/docs')).toBe(true);
    });
  ...
  describe('normalizePathname', () => {
    ...
    it('should handle complex URLs', () => {
      expect(normalizePathname('/features/?tab=advanced#section1')).toBe(
        '/features'
      );
    });
  });
  ...
  describe('isExternalLink', () => {
    it('should identify external links by URL', () => {
      expect(
        isExternalLink({ label: 'Google', href: 'https://google.com' })
      ).toBe(true);
      expect(
        isExternalLink({ label: 'Example', href: 'http://example.com' })
      ).toBe(true);
    });

    it('should identify external links by flag', () => {
      expect(
        isExternalLink({ label: 'Status', href: '/status', external: true })
      ).toBe(true);
    });

    it('should identify internal links', () => {
      expect(isExternalLink({ label: 'Features', href: '/features' })).toBe(
        false
      );
  ...
  describe('getLinkProps', () => {
    ...
    it('should return correct props for internal links', () => {
      const internalItem: NavigationItem = {
        label: 'Features',
        href: '/features',
      };
    ...
    it('should handle external flag correctly', () => {
      const flaggedItem: NavigationItem = {
        label: 'Status',
        href: '/status',
        external: true,
      };
